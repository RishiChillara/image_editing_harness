...
def request_edit_plan(
    ...
    system_prompt = (
        ...
        "Also assess whether the image is worth keeping after editing using worth_saving from 0 to 1 "
        "(1 = definitely keep, 0 = not worth saving). "
        "Consider focus/sharpness, recoverable exposure, composition/subject, duplicates, and accidental frames. "
        "Set discard_reason to a short explanation when worth_saving is low; otherwise use an empty string. "
        "Always return full edit deltas even when worth_saving is low. "
